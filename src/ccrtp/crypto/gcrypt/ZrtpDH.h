...
private:
    ...
    int32_t gcryptComputeSecret(uint8_t* secret, gcry_mpi_t pubKeyOther, int32_t len);

public:
    ZrtpDH(int32_t pLength);
    ~ZrtpDH();
     ...
     */
    int32_t getSecretSize() const { return ((pLength + 7) / 8); }
     ...
     * @return Size in bytes.
     */
    int32_t getPubKeySize() const { return ((gcry_mpi_get_nbits(pubKey) + 7) / 8); }
